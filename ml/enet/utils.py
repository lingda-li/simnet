...
def calculate_1d_output_size(input_size, stride):
...
def drop_1d_connect(inputs, p, training):
    """Drop connect.
...
def e1d_params(model_name):
    ...
    params_dict = {
        # Coefficients:   width,depth,res,dropout
        'e1d-b0': (1.0, 1.0, 224, 0.2),
        'e1d-b1': (1.0, 1.1, 240, 0.2),
        'e1d-b2': (1.1, 1.2, 260, 0.3),
        'e1d-b3': (1.2, 1.4, 300, 0.3),
        'e1d-b4': (1.4, 1.8, 380, 0.4),
        'e1d-b5': (1.6, 2.2, 456, 0.4),
        'e1d-b6': (1.8, 2.6, 528, 0.5),
        'e1d-b7': (2.0, 3.1, 600, 0.5),
        'e1d-b8': (2.2, 3.6, 672, 0.5),
        'e1d-l2': (4.3, 5.3, 800, 0.5),
    }
    return params_dict[model_name]
...
def get_same_padding_conv1d(input_size=None):
    assert input_size is not None
    return partial(Conv1dStaticSamePadding, input_size=input_size)


class Conv1dStaticSamePadding(nn.Conv1d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, input_size=None, **kwargs):
        super().__init__(in_channels, out_channels, kernel_size, stride, **kwargs)
        stride = stride if isinstance(stride, int) else stride[0]
        conv_num = int(math.ceil((input_size - kernel_size) / stride) + 1)
        pad = stride * (conv_num - 1) + kernel_size - input_size
        assert pad >= 0
        if stride == 1:
            assert pad == 0
            pad = kernel_size - 1
        print("in, k, s, and pad:", input_size, kernel_size, stride, pad)
        if pad > 0:
            self.static_padding = nn.ConstantPad1d((0, pad), 0)
        else:
            self.static_padding = nn.Identity()
