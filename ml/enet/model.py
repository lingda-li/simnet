...
class Conv1DBlock(nn.Module):
    ...
    def __init__(self, block_args, global_params, input_size):
        super().__init__()
        ...
        if self._block_args.expand_ratio != 1:
            self._expand_conv = nn.Conv1d(in_channels=inp, out_channels=oup, kernel_size=1, bias=False)
        ...
        if self.has_se:
            ...
            self._se_reduce = nn.Conv1d(in_channels=oup, out_channels=num_squeezed_channels, kernel_size=1)
            self._se_expand = nn.Conv1d(in_channels=num_squeezed_channels, out_channels=oup, kernel_size=1)

        # Pointwise convolution phase
        final_oup = self._block_args.output_filters
        self._project_conv = nn.Conv1d(in_channels=oup, out_channels=final_oup, kernel_size=1, bias=False)
    ...
    def forward(self, inputs, drop_connect_rate=None):
        ...
        if self.id_skip and self._block_args.stride == 1 and input_filters == output_filters:
            ...
            if drop_connect_rate:
                x = drop_1d_connect(x, p=drop_connect_rate, training=self.training)
...
class E1DNet(nn.Module):
    ...
    Example:
        ...
        >>> inputs = torch.rand(1, context_length, inst_length) -> reversed dimensions
    ...
    def __init__(self, blocks_args=None, global_params=None):
        ...
        for block_args in self._blocks_args:
            ...
            self._blocks.append(Conv1DBlock(block_args, self._global_params, input_size=input_size))
            input_size = calculate_1d_output_size(input_size, block_args.stride)
            if block_args.num_repeat > 1: # modify block_args to keep same output size
                block_args = block_args._replace(input_filters=block_args.output_filters, stride=1)
            for _ in range(block_args.num_repeat - 1):
                self._blocks.append(Conv1DBlock(block_args, self._global_params, input_size=input_size))
                # input_size = calculate_output_input_size(input_size, block_args.stride)  # stride = 1

        # Head
        in_channels = block_args.output_filters  # output of final block
        out_channels = round_filters(1280, self._global_params)
        self._conv_head = nn.Conv1d(in_channels, out_channels, kernel_size=1, bias=False)
    ...
    def from_pretrained(cls, model_name, weights_path=None, advprop=False,
                        in_channels=3, num_classes=22, **override_params):
        """create an efficientnet model according to name.
        ...
        load_pretrained_weights(model, model_name, weights_path=weights_path, load_fc=(num_classes == 22), advprop=advprop)
        return model
