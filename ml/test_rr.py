...
def save_ts_model(name, model, device):
    ...
    model.eval()
    traced_script_module = torch.jit.trace(model, (torch.rand(1, context_length * inst_length).to(device), torch.rand(1, 1).to(device)))
...
def main():
    ...
    parser.add_argument('--batch-size', type=int, default=1024*64*3, metavar='N',
                        help='input batch size (default: 1024*64*3)')
    ...
    dataset = CombinedDataset(test_start, test_end, num_classes=num_classes)
