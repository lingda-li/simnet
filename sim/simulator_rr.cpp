...
int main(int argc, char *argv[]) {
#ifdef CLASSIFY
  if (argc < 7) {
    cerr << "Usage: ./simulator_rr <trace> <aux trace> <lat module> <class module> <# inst> <rob>" << endl;
#else
  if (argc < 6) {
    cerr << "Usage: ./simulator_rr <trace> <aux trace> <module> <# inst> <rob>" << endl;
#endif
    return 0;
  }
  ...
  float rob_para = atof(argv[arg_idx++]);
  ...
  while(!eof || !rob->is_empty()) {
    ...
    while (curTick >= nextFetchTick && !rob->is_full() && !eof) {
      ...
      inputs.push_back(para_input.cuda());
#else
      inputs.push_back(input);
      inputs.push_back(para_input);
#endif
