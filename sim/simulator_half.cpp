...
int main(int argc, char *argv[]) {
  ...
  // auto half = (c10::kHalf).scalarTensor(0.0);
  float *inputPtr = input.data_ptr<float>();
  // at::Half *half = input_half.data_ptr<at::Half>();
...
#ifdef HALF
    // input.data_ptr<at::Half>();   // working code line
    // cout<<"Attempling data conversion to half...\n";
    // cout<<"casting\n";
    at::Tensor half= torch::ones({1, ML_SIZE}, torch::dtype(torch::kF16));
    size_t x_size = input.numel();
    // cout<<"Size: "<<x_size<<"\n";
    
    for(size_t i=0;i<x_size;i++) 
      {
        // std::cout << input[0][i].item<at::Half>() << std::endl;
        half[0][i]= input[0][i].item<at::Half>();
        // input_half.p( {0,i},input[0][i].item<at::Half>());
      }

    // cout<<z<<"\n";
    // std::cout << (double)output[i][0][1] << '\n';
    // input.data_ptr<c10::ScalarType::Half>();
    // cout<<"Converted data to half\n";
    // inputs.data<torch::at::Half>();
    // input.data<__half>();
    // input.data<torch::at::Half>();
#endif
#ifdef DUMP_ML_INPUT
      cout << half << "\n";
#endif
#ifdef GPU
      inputs.push_back(half.cuda());
#else
